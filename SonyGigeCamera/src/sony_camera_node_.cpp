...
//        nh = ros::NodeHandle("~");
//        it(nh);
        rgb_image = it.advertise("GigCam_rgb_image", 10);
        connectCamera();
        startStreaming();
    }
...
void Sony_Camera_Node::connectCamera(){
    ...
    lResult = lDevice.Connect( address, PvAccessControl );
...
void Sony_Camera_Node::publishImage()// image_transport::Publisher pub_rgb_mage)
{
    ...
    if ( lResult.IsOK() )
    {
        if(lOperationResult.IsOK())
        {
           ...
           // pub_rgb_mage.publish(out_msg.toImageMsg());
...
//    image_transport::Publisher pub_rgb_mage = it.advertise("GigCam_rgb_image", 10);
